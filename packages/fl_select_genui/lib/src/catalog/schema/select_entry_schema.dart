...
abstract final class SelectEntrySchema {
  ...
  static Schema node() => S.object(
        description: 'One select entry node. `type` discriminates the node: '
        ...
        properties: {
          ...
          'id': S.string(
            description: 'Stable identifier used in query results. Required '
                'for category/text/range; `any`/`custom` use fixed ids.',
          ),
          'name': S.string(description: 'Human-visible label.'),
          'min': S.number(
              description: 'Inclusive lower bound (range/any/custom).'),
          'max': S.number(
              description: 'Inclusive upper bound (range/any/custom).'),
          ...
          'layout': S.object(
            description: 'category only: {"kind": "list"|"grid"|"chip"|"counter"'
          ...
          'children': S.list(
            description: 'Nested nodes with the same shape as this object '
